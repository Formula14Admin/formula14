...
interface DaySlot {
  dow: number; label: string; available: boolean; start: string; end: string; types: string[]
}
...
export function CoachScheduleEditor({ coachId, coachName }: { coachId: string; coachName: string }) {
  const blank = (): DaySlot[] =>
    DAYS_SHORT.map((label, i) => ({ dow: DOW_JS[i], label, available: false, start: '06:00', end: '21:00', types: [...COACHED_TYPE_IDS] }))
  ...
  useEffect(() => {
    void (async () => {
      try {
        const { data, error: err } = await supabase.from('coach_availability').select('*').eq('coach_id', coachId)
        if (err) throw err
        setDays(blank().map(slot => {
          const row = (data ?? []).find((r: Record<string, unknown>) => r.day_of_week === slot.dow)
          if (!row) return slot
          return { ...slot, available: true, start: timeStr(row.start_time as string) || '06:00', end: timeStr(row.end_time as string) || '21:00', types: (row.session_types_enabled as string[]) ?? [...COACHED_TYPE_IDS] }
        }))
  ...
  function toggle(idx: number) { setDays(prev => prev.map((d, i) => i === idx ? { ...d, available: !d.available } : d)) }
  ...
  async function save() {
    ...
    try {
      ...
      if (offDows.length > 0) {
        const { error: de } = await supabase.from('coach_availability').delete().eq('coach_id', coachId).in('day_of_week', offDows)
        if (de) throw de
      }
      for (const d of days.filter(d => d.available)) {
        const { error: ue } = await supabase.from('coach_availability').upsert({ coach_id: coachId, day_of_week: d.dow, start_time: d.start, end_time: d.end, session_types_enabled: d.types }, { onConflict: 'coach_id,day_of_week' })
        if (ue) throw ue
      }
