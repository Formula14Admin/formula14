...
import {
  ...
  INCOME_SOURCES, EXPENSE_CATS_PERSONAL, PERSONAL_MONTHLY,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard icon={<IconArrowUpRight size={16}/>} label="Income (Jun)"   value={fmtMoney(income)}   color="#10b981" />
        <StatCard icon={<IconArrowDownLeft size={16}/>} label="Expenses (Jun)" value={fmtMoney(expenses)} color="#ef4444" negative />
        <StatCard icon={<IconCheck size={16}/>}         label="Savings (Jun)"  value={fmtMoney(savings)}  color={savings>=0 ? '#10b981' : '#ef4444'} negative={savings<0} />
          ...
          <table className="w-full min-w-[400px] text-sm">
            ...
            <tbody>
              {PERSONAL_MONTHLY.map((m, i) => (
                <tr key={i} className={`border-b border-gray-50 last:border-0 ${'partial' in m && m.partial ? 'opacity-60' : ''} hover:bg-gray-50`}>
                  <td className="px-4 py-2.5 font-medium text-gray-700">{m.month}{'partial' in m && m.partial && <span className="ml-1 text-[10px] text-gray-400">*</span>}</td>
