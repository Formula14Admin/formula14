-- Migration 022: Fix register_user to handle orphaned users rows.
-- An orphaned user is one where the linked athletes row was deleted
-- (e.g. via an old delete path that didn't clean up the users row).
-- Instead of blocking re-registration, we delete the orphaned row and proceed.
...
BEGIN
  ...
  IF v_existing_id IS NOT NULL THEN
    ...
    ELSE
      ...
      DELETE FROM users WHERE id = v_existing_id;
    END IF;
  END IF;
